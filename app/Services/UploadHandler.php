...
class UploadHandler {
	...
	public function getName() {
		...
		if (isset($_FILES[$this->inputName])) return $_FILES[$this->inputName]['name'];
	}
	...
	public function handleUpload($uploadDirectory, $name = null) {
		...
		$name = Strings::webalize($name) . "." . array_pop($exploded);
		...
		if ($totalParts > 1) {
			...
			return array("success" => true, "uuid" => $uuid, "name" => $name);
	...
	public function handleDelete($uploadDirectory, $name = null) {
		...
		if (is_dir($target)) {
			...
			return array("success" => false, "error" => "File not found! Unable to delete." . $url, "path" => $uuid);
		}
